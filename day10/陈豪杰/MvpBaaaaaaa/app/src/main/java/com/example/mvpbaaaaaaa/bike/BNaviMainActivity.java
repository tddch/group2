package com.example.mvpbaaaaaaa.bike;
...
public class BNaviMainActivity extends Activity {
    ...
    public void startBikeNavi() {
        ...
        try {
            // 获取导航控制类
            // 引擎初始化
            BikeNavigateHelper.getInstance().initNaviEngine(BNaviMainActivity.this, new IBEngineInitListener() {
                @Override
    ...
    private void routePlanWithBikeParam() {
        ...
        BikeRouteNodeInfo bikeEndNode = new BikeRouteNodeInfo();
        bikeEndNode.setLocation(endPt);
        bikeParam = new BikeNaviLaunchParam().startNodeInfo(bikeStartNode).endNodeInfo(bikeEndNode).vehicle(0);
        BikeNavigateHelper.getInstance().routePlanWithRouteNode(bikeParam, new IBRoutePlanListener() {
            @Override
            public void onRoutePlanStart() {
                Log.d(TAG, "BikeNavi onRoutePlanStart执行算路开始的逻辑");
            }
            ...
            public void onRoutePlanFail(BikeRoutePlanError error) {
                Log.d(TAG, "BikeNavi onRoutePlanFail执行算路失败的逻辑");
            }
